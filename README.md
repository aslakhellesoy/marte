...
At build time `malte` renders each block and injects it into the matching
...
component and Markdown companion (malte composes per component).
...
- `extract` adds `data-malte` markers to a component's static prose and writes the
...
Markdown — ideal for blog posts. `malte` is the inverse: it keeps prose _out of_
